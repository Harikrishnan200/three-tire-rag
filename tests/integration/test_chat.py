...
async def test_chat_creates_conversation_and_returns_answer(
    client, graph_repository, llm_provider
) -> None:
    ...
    response = await client.post(
        ...
        json={"conversation_id": None, "message": "Who is Microsoft's current CEO?"},
    )
...
async def test_chat_response_is_cached(client, graph_repository) -> None:
    ...
    r1 = await client.post(
        "/api/v1/chat",
        headers=auth_headers(token),
        json={"conversation_id": None, "message": "Who is Microsoft's CEO?"},
    )
